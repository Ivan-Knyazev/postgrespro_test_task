...
3) **Проверка подключения к `postgres` на серверах с локальной машины** Для тестирования корректной работы настроенного `pg_hba.conf` файла, была произведена попытка подключения к каждому из серверов с локального ПК. Как можно видеть на скриншоте, всё корректно работает, подключение не удаётся.
   
   ![test_1](./images/image_3.png)
